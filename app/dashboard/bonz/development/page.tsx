...
export default function DevelopmentPage() {
  ...
  const fetchDevelopment = useCallback(async () => {
    try {
      ...
      const developmentData = res.data.data || res.data;
      const formattedDevelopment = developmentData.map((item: DevelopmentItem) => ({
        ...item,
        contents: typeof item.contents === "object" 
          ? jsonToHTML(item.contents as Record<string, unknown>)
          : item.contents,
      }));
      setDevelopment(formattedDevelopment);
    } catch (err) {
      console.error("Fetch development error:", err);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleDelete = async (id: string) => {
    ...
    if (!confirmed) return;
    try {
      await axios.delete(`${API_URL}/api/development/${id}`);
      setDevelopment(development.filter((item) => item.id !== id));
    } catch (err) {
  ...
  const handleSave = async () => {
    try {
      ...
      if (editId) {
        const res = await axios.put(`${API_URL}/api/development/${editId}`, payload);
        ...
        setDevelopment([{ ...newItem, contents: newContents }, ...development]);
      }
  ...
  const filteredDevelopment = development.filter((item) =>
                  ...
                  <div>
                    ...
                    {item.updated_at && (
                      <p className="text-xs text-green-600 mt-1">
                        Сүүлд шинэчилсэн: {new Date(item.updated_at).toLocaleDateString()}
                      </p>
                    )}
