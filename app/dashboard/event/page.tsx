    ...
    try {
      ...
      if (editId) {
        ...
        const res = await axios.put(
          `http://localhost:5001/api/event/${editId}`,
          payload
        );
        setNews(news.map((item) => (item.id === editId ? res.data : item)));
      } else {
        // Add mode
        const res = await axios.post("http://localhost:5001/api/event", {
          title: newTitle,
          description: newDescription,
          images,
          is_research: false,
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex justify-between items-center mb-4">
        <input
          ...
          className="w-[300px] p-3 rounded-xl border bg-background/50 text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-blue-500 transition"
        />
        <Button
