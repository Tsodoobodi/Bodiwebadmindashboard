...
export default function InnovationPage() {
  const [news, setNews] = useState<InnovationItem[]>([]);
  ...
  const fetchNews = async () => {
    try {
      setLoading(true);
      const res = await axios.get("http://localhost:5001/api/innovation", {
  ...
  const handleDelete = async (id: string) => {
    ...
    if (!confirmed) return;
    try {
      await axios.delete(`http://localhost:5001/api/innovation/${id}`);
  ...
  const handleSave = async () => {
    try {
      ...
      if (editId) {
        ...
        // Хэрвээ editDate өөрчлөгдсөн бол updated_date-д илгээнэ
        if (editDate) {
          payload.updated_date = new Date(editDate).toISOString();
        }
        const res = await axios.put(
          `http://localhost:5001/api/innovation/${editId}`,
          payload
        );
        setNews(news.map((item) => (item.id === editId ? res.data : item)));
      } else {
        // Add mode
        const res = await axios.post("http://localhost:5001/api/innovation", {
          title: newTitle,
          description: newDescription,
          images,
          is_research: false,
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex items-center mb-4 gap-6">
        <input
          type="text"
          placeholder="Инновац мэдээ хайх ..."
          value={query}
                ...
                <div className="p-4 flex-1 flex flex-col justify-between">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      Оруулсан: {new Date(item.created_date).toLocaleDateString()}
                    </p>
                    {item.updated_date && (
