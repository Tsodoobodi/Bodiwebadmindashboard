...
export default function ResearchPage() {
  ...
  const fetchResearch = useCallback(async () => {
    try {
      ...
      const formattedResearch = researchData.map((item: ResearchItem) => ({
        ...item,
  ...
  const handleSave = async () => {
    try {
      ...
      if (editId) {
        const res = await axios.put(`${API_URL}/api/research/${editId}`, payload);
          ...
          <div className="bg-background w-[90vw] h-[90vh] max-w-screen max-h-screen rounded-2xl shadow-xl p-6 flex flex-col">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-bold">
                {editId ? "Мэдээ засах" : "Research нэмэх"}
              </h2>
              <Button
                variant="outline"
                size="sm"
