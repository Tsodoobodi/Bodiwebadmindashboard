...
interface RndglobalstrategyItem {
  id: string;
  title: string;
...
export default function RndglobalstrategyPage() {
  ...
  const fetchRndglobalstrategy = useCallback(async () => {
    try {
      setLoading(true);
      const res = await axios.get(`${API_URL}/api/rndglobalstrategy`);
      const data = res.data.data || res.data;
      const formatted = data.map((item: RndglobalstrategyItem) => ({
        ...item,
        contents: typeof item.contents === "object" 
          ? jsonToHTML(item.contents as Record<string, unknown>)
          : item.contents,
      }));
      setRndglobalstrategy(formatted);
    } catch (err) {
  ...
  const handleDelete = async (id: string) => {
    ...
    if (!confirmed) return;
    try {
      await axios.delete(`${API_URL}/api/rndglobalstrategy/${id}`);
      setRndglobalstrategy(rndglobalstrategy.filter((item) => item.id !== id));
    } catch (err) {
  ...
  const handleSave = async () => {
    try {
      ...
      if (editId) {
        const res = await axios.put(`${API_URL}/api/rndglobalstrategy/${editId}`, payload);
        const updatedItem = res.data.data || res.data;
        setRndglobalstrategy(
          rndglobalstrategy.map((item) =>
            item.id === editId ? { ...updatedItem, contents: newContents } : item
          )
        );
      } else {
        const res = await axios.post(`${API_URL}/api/rndglobalstrategy`, payload);
        const newItem = res.data.data || res.data;
        setRndglobalstrategy([{ ...newItem, contents: newContents }, ...rndglobalstrategy]);
      }
  ...
  const filtered = rndglobalstrategy.filter((item) =>
              ...
              <div key={item.id} className="bg-card rounded-2xl shadow-md overflow-hidden cursor-pointer flex flex-col transition-all duration-300 ease-in-out hover:scale-100 hover:shadow-xl h-[400px]">
                ...
                <div className="p-4 flex-1 flex flex-col justify-between">
                  ...
                  <div className="mt-4 flex gap-2">
                    ...
                    <Button size="sm" variant="destructive" onClick={() => handleDelete(item.id)}>Устгах</Button>
                  </div>
                </div>
              </div>
            );
