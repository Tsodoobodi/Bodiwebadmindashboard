...
export default function RndprofessionalPage() {
  const [rndprofessional, setRndprofessional] = useState<RndprofessionalItem[]>([]);
  ...
  const fetchRndprofessional = useCallback(async () => {
    try {
      setLoading(true);
      const res = await axios.get(`${API_URL}/api/rndprofessional`);
      const data = res.data.data || res.data;
      const formatted = data.map((item: RndprofessionalItem) => ({
        ...item,
        contents: typeof item.contents === "object" 
          ? jsonToHTML(item.contents as Record<string, unknown>)
          : item.contents,
      }));
      setRndprofessional(formatted);
    } catch (err) {
  ...
  const handleDelete = async (id: string) => {
    ...
    if (!confirmed) return;
    try {
      await axios.delete(`${API_URL}/api/rndprofessional/${id}`);
      setRndprofessional(rndprofessional.filter((item) => item.id !== id));
    } catch (err) {
  ...
  const handleSave = async () => {
    try {
      ...
      if (editId) {
        const res = await axios.put(`${API_URL}/api/rndprofessional/${editId}`, payload);
        const updatedItem = res.data.data || res.data;
        setRndprofessional(
          rndprofessional.map((item) =>
            item.id === editId ? { ...updatedItem, contents: newContents } : item
          )
        );
      } else {
        const res = await axios.post(`${API_URL}/api/rndprofessional`, payload);
        const newItem = res.data.data || res.data;
        setRndprofessional([{ ...newItem, contents: newContents }, ...rndprofessional]);
      }
  ...
  const filtered = rndprofessional.filter((item) => {
    ...
    <div className="flex flex-col gap-6">
      <div className="flex flex-col gap-4">
        <div className="flex gap-4 items-center flex-wrap">
          ...
          <Button
            className="bg-none rounded-xl w-[200px] h-10 text-md border border-gray-500 hover:bg-gray-500 hover:text-white transition-all"
            onClick={() => {
              setOpen(true);
              setEditId(null);
              setNewTitle("");
        ...
        <div className="flex gap-4 items-center flex-wrap">
          ...
          <span className="text-sm text-muted-foreground">
            {filtered.length} / {rndprofessional.length} мэдээ
          </span>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((item) => {
            ...
            return (
              <div key={item.id} className="bg-card rounded-2xl shadow-md overflow-hidden flex flex-col transition-all hover:shadow-xl h-[400px]">
                {firstImg && (
