...
export default function PresentationItem() {
  const [news, setNews] = useState<PresentationItem[]>([]);
  ...
  const fetchNews = async () => {
    try {
      setLoading(true);
      const res = await axios.get("http://localhost:5001/api/presentation", {
  ...
  const handleDelete = async (id: string) => {
    ...
    if (!confirmed) return;
    try {
      const response = await axios.delete(`http://localhost:5001/api/presentation/${id}`);
      console.log('Delete response:', response.status);
      setNews(news.filter((item) => item.id !== id));
      alert("Амжилттай устгагдлаа!");
    } catch (err) {
      console.error('Delete error:', err);
      alert("Устгахад алдаа гарлаа. Backend серверийг шалгана уу.");
    }
  };
    ...
    try {
      ...
      if (editId) {
        ...
        const res = await axios.put(
          `http://localhost:5001/api/presentation/${editId}`,
          payload
        );
        setNews(news.map((item) => (item.id === editId ? res.data : item)));
      } else {
        // Add mode
        const res = await axios.post("http://localhost:5001/api/presentation", {
          title: newTitle,
          description: newDescription,
          images,
          is_research: false,
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex items-center mb-4 gap-4">
        <input
          type="text"
          placeholder="Танилцуулага мэдээ хайх ..."
          value={query}
                ...
                <div className="p-4 flex-1 flex flex-col justify-between">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      Оруулсан: {item.created_date ? new Date(item.created_date).toLocaleDateString() : "Тодорхойгүй"}
                    </p>
                    {item.updated_date && (
