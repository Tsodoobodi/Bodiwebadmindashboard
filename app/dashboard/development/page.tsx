...
export default function NewsPage() {
  ...
  const fetchNews = async () => {
    try {
      setLoading(true);
      const res = await axios.get("http://localhost:5001/api/development", {
  ...
  const handleDelete = async (id: string) => {
    ...
    if (!confirmed) return;
    try {
      await axios.delete(`http://localhost:5001/api/development/${id}`);
    ...
    try {
      ...
      if (editId) {
        ...
        const res = await axios.put(
          `http://localhost:5001/api/development/${editId}`,
          payload
        );
        setNews(news.map((item) => (item.id === editId ? res.data : item)));
      } else {
        // Add mode
        const res = await axios.post("http://localhost:5001/api/development", {
          title: newTitle,
          description: newDescription,
          images,
          is_research: false,
